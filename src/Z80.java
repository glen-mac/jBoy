...
public class Z80 {
	...
	/* TO DO:
	1) ADD IN PARAMx VARIABLES IN EXECUTE FUNCTION INSTEAD OF READING BYTES 
	*/
...
private void add(int r1, int data){	
	...
	if (((registers[r1] & 0x0F) + (data & 0x0F)) > 0x0F)
		setFlag(FLAG_HALFCARRY);
...
private void addHL(int data){
	...
	if (((comb & 0xFFF) + (data & 0xFFF)) > 0xFFF)
		setFlag(FLAG_HALFCARRY);
